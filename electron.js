...
function handleSquirrelEvent() {
    ...
    switch (squirrelEvent) {
        case '--squirrel-install':
            try {
                childProcess.spawn(target, ['--createShortcut', `${app.getName()}.exe`], {
                    detached: true
                });

                // 🟡 OPTIONAL: Create registry keys, analytics, logs, etc.
                console.log("🟢 First install detected. Shortcuts created.");
            } catch (e) { }
            return false;
        case '--squirrel-updated':
            const target = path.resolve(process.execPath, '..', '..', '..', 'Update.exe');
            const appName = 'BreakEven Installer';
            try {
                childProcess.spawn(target, ['--createShortcut', `${app.getName()}.exe`], {
                    detached: true
                });
            } catch (e) { }
            return false; // Let the app continue running
        case '--squirrel-uninstall':
            try {
                const target = path.resolve(process.execPath, '..', '..', '..', 'Update.exe');
                childProcess.spawn(target, ['--removeShortcut', `${app.getName()}.exe`], {
                    detached: true
                });
            } catch (e) { }
            app.quit();
...
if (handleSquirrelEvent()) {
    // Squirrel event handled and app.quit() already called
    return;
}
*/
